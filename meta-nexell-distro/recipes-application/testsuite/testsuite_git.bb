...
#    oe_runmake CROSS_COMPILE=${TARGET_PREFIX} INCLUDE="-I${STAGING_DIR_TCBOOTSTRAP}/usr/include -I${S}/libnx_video_alloc/include -I${S}/libnx_video_alloc/src" LIBRARY="-L${S}/libnx_video_alloc/src -lnx_video_alloc" LDFLAGS="-L${STAGING_LIBDIR}" CC="$CC"
    ...
    cd ${S}/usb_test
    oe_runmake CROSS_COMPILE=${TARGET_PREFIX} CC="$CC" clean
    oe_runmake CROSS_COMPILE=${TARGET_PREFIX} INCLUDES="-I${STAGING_INCDIR} -I${STAGING_INCDIR}/libusb-1.0" LDFLAGS="-L${STAGING_LIBDIR} -L{STAGING_LIBDIR}/lib/" CC="$CC"

    #oe_runmake CROSS_COMPILE=${TARGET_PREFIX} INCLUDES="-I${STAGING_INCDIR} -I${STAGING_INCDIR}/libusb-1.0 -I${S}/usb_test/include/ -I" LDFLAGS="-L${STAGING_LIBDIR} -L{STAGING_LIBDIR}/lib/ -L${S}/usb_test/lib/" CC="$CC"
...
do_install() {
    ...
    #usb_test
    install -m 0755 ${S}/usb_test/usb_test ${D}${bindir}
}
