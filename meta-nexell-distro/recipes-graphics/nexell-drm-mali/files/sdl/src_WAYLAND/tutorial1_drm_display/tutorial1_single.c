...
static struct drm_fb * drm_fb_get_from_bo(struct gbm_bo *bo)
{
	...
	temp_bo = gbm_bo_get_handle(bo);
	printf("handle: %d, %d, %d\n", gbm_bo_get_handle(bo).u32, gbm_bo_get_handle(bo), VR_LOW_32_OF_64(temp));
...
int main(int argc, char *argv[])
{
	...
	/* Initialize EGL. */
	initializeEGL(gbm.dev, (EGLNativeWindowType)gbm.surface);
	#else
	ret = init_gl();
	if (ret) {
	...
	printf("gbm_surface_lock_front_buffer start.\n");

	bo = gbm_surface_lock_front_buffer(pStatics->sWindow);

	printf("drm_fb_get_from_bo start.\n");

	fb = drm_fb_get_from_bo(bo);

	printf("drm_fb_get_from_bo done.\n");
	...
	if(!InitOGLES()) return -1; //OpenGL ES Initialization
