...
static struct gbm_device *g_gbm_device;
...
static void setup_opengl () {
	g_gbm_device = gbm_create_device (device);
	display = eglGetDisplay (g_gbm_device);
	...
	g_gbm_surface = gbm_surface_create (g_gbm_device, mode_info.hdisplay, mode_info.vdisplay, GBM_BO_FORMAT_XRGB8888, GBM_BO_USE_SCANOUT|GBM_BO_USE_RENDERING);
	egl_surface = eglCreateWindowSurface (display, config, g_gbm_surface, NULL);
...
static void swap_buffers () {
	...
	if (previous_bo) {
		...
		gbm_surface_release_buffer (g_gbm_surface, previous_bo);
	}
	previous_bo = bo;
	previous_fb = fb;
}
