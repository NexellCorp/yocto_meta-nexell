...
make_boot_image() {
	...
	local kern_img=${deploy_dir}/${KERNEL_IMAGETYPE}
	local logo_bmp=${UBOOT_LOGO_BMP}
	local dtb_file

	mkdir -p ${boot_dir}

	if [ -e ${deploy_dir}/${KERNEL_DEVICETREE} ]; then
		dtb_file=${deploy_dir}/${KERNEL_DEVICETREE}
	else
		dtb_file=${deploy_dir}/${KERNEL_IMAGETYPE}-${KERNEL_DEVICETREE}
	fi
