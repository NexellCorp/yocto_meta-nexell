...
static int write_all_blocks(struct sparse_file *s, struct output_file *out,
			void (*progress)(unsigned int perc))
{
	struct backed_block *bb;
	unsigned int last_block = 0;
	int64_t pad;
	unsigned int percent, prevpercent = 0;
	int block_count, total_block = 0;
	...
	for (bb = backed_block_iter_new(s->backed_block_list); bb;
		...
		if (progress) {
			percent = (unsigned)(100ULL *
				  (total_block - block_count) / total_block);
...
int sparse_file_write(struct sparse_file *s, int fd, bool gz, bool sparse,
		bool crc, void (*progress)(unsigned int perc))
{
	int ret;
	int chunks;
	...
	ret = write_all_blocks(s, out, progress);
		...
		int (*write)(void *priv, const void *data, int len), void *priv)
{
	...
	ret = write_all_blocks(s, out, NULL);
