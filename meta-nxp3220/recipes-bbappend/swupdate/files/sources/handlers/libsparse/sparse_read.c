...
static int64_t lseek_offs = 0;

static off64_t lseek64_offset(int fd, off64_t offset, int whence)
{
	off64_t offs = whence != SEEK_CUR ? lseek_offs : 0;

	return lseek64(fd, offset + offs, whence);
}

void sparse_file_lseek_offset(int64_t offset)
{
	lseek_offs = offset;
}
		...
		uint32_t *crc32)
{
	...
	if (crc32) {
		...
		lseek64_offset(fd, len, SEEK_CUR);
	}
...
static int sparse_file_read_sparse(struct sparse_file *s, int fd, bool crc)
{
	...
	if (sparse_header.file_hdr_sz > SPARSE_HEADER_LEN) {
		 ...
		 * we expected.
		 */
		lseek64_offset(fd, sparse_header.file_hdr_sz - SPARSE_HEADER_LEN, SEEK_CUR);
	}
	...
	for (i = 0; i < sparse_header.total_chunks; i++) {
		...
		if (sparse_header.chunk_hdr_sz > CHUNK_HEADER_LEN) {
			 ...
			 * we expected.
			 */
			lseek64_offset(fd, sparse_header.chunk_hdr_sz - CHUNK_HEADER_LEN, SEEK_CUR);
		}

		offset = lseek64_offset(fd, 0, SEEK_CUR);
...
struct sparse_file *sparse_file_import(int fd, bool verbose, bool crc)
{
	...
	ret = lseek64_offset(fd, 0, SEEK_SET);
	if (ret < 0) {
...
struct sparse_file *sparse_file_import_auto(int fd, bool crc)
{
	...
	len = lseek64_offset(fd, 0, SEEK_END);
	if (len < 0) {
		return NULL;
	}

	lseek64_offset(fd, 0, SEEK_SET);
