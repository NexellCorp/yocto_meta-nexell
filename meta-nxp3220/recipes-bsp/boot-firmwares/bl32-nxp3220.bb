...
do_deploy () {
	...
	if ${@bb.utils.contains('BINARY_FEATURES','nand','true','false',d)}; then
		if [ -z ${NAND_PAGE_SIZE_BYTE} ]; then
			echo "ERROR: NOT Defined 'NAND_PAGE_SIZE_BYTE'"
			exit 1
		fi

		# ((Encrypted Binary) + NSIH) + ECC : $BIN.enc.raw.ecc
		do_bingen_ecc ${DEPLOYDIR}/${BL32_BIN}.enc.raw ${NAND_PAGE_SIZE_BYTE}

		# (Binary + NSIH) + ECC : $BIN.raw.ecc
		do_bingen_ecc ${DEPLOYDIR}/${BL32_BIN}.raw ${NAND_PAGE_SIZE_BYTE}
	fi
}
