...
nexell_machine_name=$2
#------------------------------------------------------------------------------------------------------------------------------

if [[ $3 == "accept-eula" ]] ; then
   eula=true
fi

echo "You selected target $choice"

declare -a targets=("qemux86-64" "porter" "raspberrypi2" "raspberrypi3" "minnowboard" "silk" "dragonboard-410c", "nexell")
declare -a supported=("qemux86-64" "porter" "minnowboard" "raspberrypi2" "raspberrypi3" "dragonboard-410c" "silk" "nexell")
...
#------------------------------------------------------------------------------------------------------------------------------
# nexell modified codes
cp ../build-${nexell_machine_name}-genivi/conf/templates/${machine}.local.conf ../build-${nexell_machine_name}-genivi/conf/local.conf
cp ../build-${nexell_machine_name}-genivi/conf/templates/${machine}.bblayers.conf ../build-${nexell_machine_name}-genivi/conf/bblayers.conf
#------------------------------------------------------------------------------------------------------------------------------
...
bsparr+=( ["minnowboard"]="meta-intel" ["raspberrypi2"]="meta-raspberrypi" ["raspberrypi3"]="meta-raspberrypi" ["porter"]="meta-renesas" ["silk"]="meta-renesas" ["dragonboard-410c"]="meta-qcom" ["nexell"]="meta-nexell" )
...
source poky/oe-init-build-env ../build-${nexell_machine_name}-genivi
