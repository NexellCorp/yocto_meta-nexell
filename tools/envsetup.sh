...
declare -a targets=("s5p4418-avn-ref" "s5p4418-navi-ref" "s5p4418-daudio-ref" "s5p6818-artik710-raptor" "s5p6818-avn-ref" "s5p4418-smart-voice" "s5p6818-kick-st" "s5p4418-daudio-covi" "s5p4418-ff-voice" "s5p4418-svm-ref")
...
function local_conf_append()
{
    ...
    if [ "${SDK_RELEASE}" == "false" ]; then
        ...
        if [ ${IMAGE_TYPE} == "qt" -o ${IMAGE_TYPE} == "smartvoice" ]; then
            sed -i "/DEPENDS/d" ${META_NEXELL_PATH}/recipes-core/images/meta-environment.bbappend
            echo "DEPENDS += \"nexell-drm-mali\"" >> ${META_NEXELL_PATH}/recipes-core/images/meta-environment.bbappend
        fi
    fi
