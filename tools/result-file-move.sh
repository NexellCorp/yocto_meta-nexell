    ...
    if [ "${BOARD_SOCNAME}" == "s5p6818" ]; then
        ...
        if [ ${BOARD_NAME} == "smart-voice" -o ${BOARD_NAME} == "ff-voice" ];then
            ...
            if [ "${BOARD_NAME}" == "avn-ref" ]; then
                ...
                cp ${META_NEXELL_PATH}/tools/${MACHINE_NAME}/bl1-convergence-usb.bin ${RESULT_PATH}
            fi
...
function copy_dtb_file()
{
	...
	file_name_dtb="s5p6818-kick-st-rev*.dtb"
	kernel_image_path=${BOARD_SOCNAME}_${BOARD_PREFIX}_${BOARD_POSTFIX}-poky-linux/linux-${MACHINE_NAME}
    else
	...
	if [ ${BOARD_PREFIX} == "avn" ]; then
	    file_name_dtb="s5p4418-avn_ref*.dtb"
            ...
            if [ ${BOARD_POSTFIX} == "svmc" ]; then
                file_name_dtb="s5p4418-convergence_svmc*.dtb"  	          
	    fi
        fi
    fi
